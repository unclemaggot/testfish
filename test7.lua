...
local Window = WindUI:CreateWindow({
    Title = "e-Fishery V3.0", Author = "by Zee (WindUI Edition)", Folder = "e-Fishery",
...
for _, weather in ipairs(weathersData) do
    if weather.Name ~= "Snow" and weather.Name ~= "Shark Hunt" then -- Filter out options not in the original request
...
for _, weatherData in ipairs(weathersData) do 
    ...
    BuyWeather:Button({ Title = buttonTitle, Callback = function() 
        local remote = getNetFolder() and getNetFolder():FindFirstChild("RF/PurchaseWeatherEvent")
        if remote then
            pcall(remote.InvokeServer, remote, weatherData.Name) 
            NotifyInfo("Manual Purchase", "Sent request to buy " .. weatherData.Name)
        end
    end })
end
