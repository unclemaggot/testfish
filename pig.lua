-- =========================
-- FishIt Lite - Full Script (Auto Farm Edition)
-- =========================
...
local function startAutoSell()
    ...
    autoSellLoop = task.spawn(function()
        ...
        while featureState.AutoSell do
                ...
                if type(items) == "table" then
                    for _, item in ipairs(items) do
                        if not item.Favorited then
                            task.spawn(function()
                                pcall(sellItemFunc.InvokeServer, sellItemFunc, item.Id or item.Uuid)
                            end)
                        end
                    end
                end
            ...
            -- wait the delay before next auto sell
            local t0 = os.clock()
...
Main:CreateToggle({
    ...
    Description = "Automatically favorite all Legendary/Mythic/Secret fish.",
    CurrentValue = false,
    Callback = function(v)
...
Main:CreateToggle({
    Name = "Auto Sell (All non-favorited fish)",
    Description = "Automatically sell all non-favorited fish every X minutes.",
    CurrentValue = false,
    Callback = function(v)
...
Main:CreateInput({
    ...
    PlaceholderText = "Enter number, e.g., 5",
    Callback = function(v)
...
for _, loc in ipairs(island_locations) do
    AutoFarm:CreateButton({
        Name = loc.Name,
        Description = "Teleport to " .. loc.Name .. " and start auto farming.",
        Callback = function()
            -- Teleport
            teleportTo(loc.Position)
            state.SelectedTP = loc.Name

            -- Start Auto Fish if not already running
