...
local function startAutoSell()
    task.spawn(function()
        while state.AutoSell do
            local DataReplion = Replion.Client:WaitReplion("Data")
            local items = DataReplion and DataReplion:Get({"Inventory","Items"})
            if type(items) ~= "table" then task.wait(2) continue end
...
Main:CreateToggle({
    Name = "Auto Sell (Every 30 non-favorited fish)",
    Description = "Automatically sell all non-favorited fish once you caught 30 fish.",
    CurrentValue = false,
    Callback = function(v)
        state.AutoSell = v
